...
if __name__ == "__main__":
    ...
    if args.compare_every >= args.simsteps:
        raise ValueError(
            f"Arg --compare-every cannot be >= {args.simsteps}."
        )

    # Load template sphere mesh for the bob
    ...
    # Build parameter dict for optimization
    params = {}
    ...
    for e in trange(args.epochs):
        ...
        if float(loss_val) <= best_loss_so_far:
            ...
            if args.log:
                length_cur = float(params.get("length", jnp.array([args.length]))[0])
                gravity_cur = float(params.get("gravity", jnp.array([args.gravity]))[0])
                pend = SimplePendulum(args.mass, args.radius, gravity_cur, length_cur, args.damping)
                ret = odeint(lambda y, t: pend.forward(t, y), theta_init, times)
                theta1 = ret[:, 0]
                x = length_cur * jnp.sin(theta1)
