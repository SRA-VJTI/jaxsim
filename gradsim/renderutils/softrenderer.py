...
class SoftRenderer(object):
    ...
    def __init__(
        ...
        if camera_direction is None:
            self.camera_direction = torch.tensor([0, 0, 1.0], device=device)
        else:
        ...
        # perspective distortion and camera position and orientation.
            ...
            self.camera_direction = torch.tensor([0, 0, 1.0], device=self.device)
        ...
        if light_direction is None:
            self.light_direction = torch.tensor([0, 1.0, 0], device=device)
        else:
    ...
    def lighting(
        self, vertices: torch.Tensor, faces: torch.Tensor, textures: torch.Tensor
    ):
    ...
    def look(
        ...
        if direction is None:
            direction = torch.tensor([0, 1.0, 0], device=self.device)
        else:
    ...
    def perspective_distortion(
        self, vertices: torch.Tensor, angle: Optional[float] = 30.0
    ):
    ...
    def vertices_to_faces(self, vertices: torch.Tensor, faces: torch.Tensor):
        ...
        faces = (
            faces
            + (torch.arange(B, dtype=torch.long, device=device) * V)[:, None, None]
        )
