...
def mem_report():
    '''Report the memory usage of live JAX arrays.'''
    try:
        import jax
        arrays = jax.live_arrays()
        total_bytes = sum(a.nbytes for a in arrays)
        print('Live JAX arrays: %d \tUsed Memory Space: %.2f MBytes' % (len(arrays), total_bytes / 1024 / 1024))
    except Exception:
        print('mem_report: unable to query JAX memory')
    print('=' * 65)
